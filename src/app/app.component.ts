...
export class AppComponent implements OnInit {
  ...
  **********************************************************************/
  formattingData = (data) => {
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      object.word_minimized = data[i].body_type.toLowerCase() + data[i].car_make.toLowerCase() + data[i].car_model.toLowerCase() + data[i].car_trim.toLowerCase() + 
        data[i].car_year.toLowerCase() + data[i].cylinder.toLowerCase() + data[i].date_delivery.toLowerCase() + data[i].doors.toLowerCase() + 
        data[i].downpayment.toString() + data[i].engine.toLowerCase() + data[i].ext_color.toLowerCase() + data[i].fepm.toString() + 
        data[i].filter_status.toLowerCase() + data[i].fuel_type.toLowerCase() + data[i].horsepower.toLowerCase() + 
        data[i].interior_color.toLowerCase() + data[i].km.toLowerCase() + data[i].location_filter.toLowerCase() + data[i].market_price.toString() + 
        data[i].passengers.toString() + data[i].price.toString() + data[i].savings.toString() + data[i].seats.toLowerCase() + data[i].status.toLowerCase() + 
        data[i].traction.toLowerCase() + data[i].transmission.toLowerCase() + data[i].uber_type.toLowerCase();
  ...
  *********************************************************************/
  ...
  makeSearching = () => {
    ...
    if(this.input_filter){ // Si la variable en entrada no es null ni vacia entonces ...
      ...
      for (let j = 0; j < split_filters.length; j++) {
        ...
        else { // Solamente el usuario coloco un filtro
